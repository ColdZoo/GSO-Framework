...
def processImage(path, out_dir):
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)  # Create Dir
    ...
    try:
        ...
        frameCount = i
    if frameCount <= 5:
        print 'ERROR, gif too short!!!'
        os.removedirs(out_dir)
        return -1
    series = [0, int(math.floor(frameCount * 0.2)), int(math.floor(frameCount * 0.4)), int(math.floor(frameCount * 0.6)),
              int(math.floor(frameCount * 0.8)), frameCount-1]
    print series
    ...
    try:
        while True:
            ...
            new_frame.save(out_dir + '/' + ''.join(os.path.basename(path).split('.')[:-1]) + '-' +
                           str(i) + '.jpg', 'JPEG')
    ...
    for j in range(0, frameCount):
        if j not in series:
            os.remove(out_dir + '/' + ''.join(os.path.basename(path).split('.')[:-1]) + '-' + str(j) + '.jpg')
        else:
            #change name
            os.rename(out_dir + '/' + ''.join(os.path.basename(path).split('.')[:-1]) + '-' + str(j) + '.jpg',
                      out_dir + '/' + ''.join(os.path.basename(path).split('.')[:-1]) + '-' + str(count) + '.jpg')
            count += 1
